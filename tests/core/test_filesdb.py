...
def df_with_duplicates() -> pda.DataFrame:
    return pda.DataFrame.from_records([(1, 2, 'v1', 'Expert', 20250302),
...
def test_deduplication(df_with_duplicates: pda.DataFrame):
    deduplicator = Deduplicator(auto_pick_last=('version', 'production_date'),
    ...
    assert df_with_duplicates.iloc[[
        4, 5
    ]].reset_index(drop=True).equals(df_no_duplicates)
...
def test_deduplicator_empty():
    deduplicator = Deduplicator(auto_pick_last=('version', 'production_date'),
                                unique=('cycle_number', 'pass_number'))
    df = deduplicator(
        pda.DataFrame(columns=('cycle_number', 'pass_number', 'version',
                               'production_date')))
    assert len(df) == 0
...
def test_unmixing_failed(df_with_duplicates: pda.DataFrame,
                         auto_pick: tuple[str], message: str):
    unmixer = SubsetsUnmixer(partition_keys=('version', 'product'),
                             auto_pick_last=auto_pick)
...
@pytest.mark.parametrize('auto_pick, group_names',
                         [(('product', 'version'), ('v1', 'Unsmoothed')),
                          (('version', 'product'), ('v2', 'Expert'))])
def test_unmixing_auto_pick(df_with_duplicates: pda.DataFrame,
...
def test_unmixing_manual_pick(df_with_duplicates: pda.DataFrame):
    ...
    df = unmixer(
        df_with_duplicates[df_with_duplicates['product'] == 'Expert'].copy())
    subset = (df_with_duplicates['version']
              == 'v2') & (df_with_duplicates['product'] == 'Expert')
    assert df.equals(df_with_duplicates[subset])
...
def test_unmixing_callable(df_with_duplicates: pda.DataFrame):
    ...
    df = unmixer(df_with_duplicates)
    subset = (df_with_duplicates['version']
              == 'v1') & (df_with_duplicates['product'] == 'Unsmoothed')
    assert df.equals(df_with_duplicates[subset])

    # We reverse the product column sort internally -> selecting Expert instead
    # of Unsmoothed
    unmixer = SubsetsUnmixer(partition_keys={
        'version': None,
...
def test_metadata_nominal(db_with_files: FilesDatabaseTest):
    assert db_with_files.variables_info(
        a_number=1) == 'metadata_from_reader_memory'
...
def test_list_files_predicate(db_with_files: FilesDatabaseTest,
                              db_predicate: FilesDatabaseTestPredicate):
    expected = pda.DataFrame(
        [(np.datetime64('2025-01-01'), 2, '/a_file_002_20250101.nc'),
...
@pytest.mark.parametrize('parameter, value', [('c_number', 10),
                                              ('unmix', False),
                                              ('deduplicate', False),
                                              ('sort', False)])
def test_query_wrong_parameter(db_with_files: FilesDatabaseTest,
                               parameter: str, value: int | bool):
    with pytest.raises(ValueError):
...
def test_query(db_with_files: FilesDatabaseTest):
    ...
    assert set(ds) == {'a', 'b'}
    assert all(ds['a'].values == [1., 1.])
    assert all(ds['b'].values == [1., 1.])
...
def test_query_reader_arg(db_with_files: FilesDatabaseTest):
    ...
    assert set(ds) == {'a', 'b'}
    assert all(ds['a'].values == [2., 2.])
    assert all(ds['b'].values == [2., 2.])
...
def test_query_metadata_injection_unknown_dim(
        db_bad_dim: FilesDatabaseTestBadDim):
    with pytest.raises(ValueError):
...
def test_map_no_dask(monkeypatch: pytest.MonkeyPatch,
...
@pytest.mark.parametrize('parameter, value', [('c_number', 10),
                                              ...
                                              ('deduplicate', False),
                                              ('sort', False)])
def test_map_wrong_parameter(db_with_files: FilesDatabaseTest, parameter: str,
                             value: int | bool):
    with pytest.raises(ValueError):
