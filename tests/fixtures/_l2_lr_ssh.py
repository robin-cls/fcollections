    ...
    return GroupMetadata(name='/',
                         variables=[
                             ...
                             VariableMetadata('latitude', float,
                                              ('num_lines', 'num_pixels'), {}),
                             VariableMetadata('ssha_karin_2', float,
                                              ('num_lines', 'num_pixels'), {}),
                         ],
                         dimensions={
                             'num_lines': 9860,
...
def _swath_properties(pass_number: int,
                      phase: str = 'calval') -> tuple[float, float, float]:
...
@pytest.fixture(scope='session')
def l2_lr_ssh_basic_dataset(
        _l2_lr_ssh_basic_metadata: GroupMetadata) -> xr.Dataset:
...
def _l2_lr_ssh_expert_metadata(
        _l2_lr_ssh_basic_metadata: GroupMetadata) -> GroupMetadata:
    metadata = copy.deepcopy(_l2_lr_ssh_basic_metadata)
    metadata.variables.extend([
        VariableMetadata('cross_track_distance', float,
                         ('num_lines', 'num_pixels'), {})
    ])
    return metadata
...
class ExpertGenerator(BasicGenerator):
    ...
    def cross_track_distance(
            self, shape: tuple[int, int]) -> np_t.NDArray[np.float64]:
        num_lines, num_pixels = shape
        return np.repeat(
            np.arange(-(num_pixels - 1) * 1000, num_pixels * 1000 + 1, 2000),
            num_lines).reshape(num_pixels, num_lines).T


@pytest.fixture(scope='session')
def l2_lr_ssh_expert_dataset(
        _l2_lr_ssh_expert_metadata: GroupMetadata) -> xr.Dataset:
...
@pytest.fixture(scope='session')
def l2_lr_ssh_unsmoothed_dataset(
        l2_lr_ssh_expert_dataset: xr.Dataset) -> xr.Dataset:
...
@pytest.fixture(scope='session')
def l2_lr_ssh_dir_empty_files(tmp_path_factory: pytest.TempPathFactory,
                              l2_lr_ssh_files: list[str]) -> Path:
...
@pytest.fixture(scope='session')
def l2_lr_ssh_dir_empty_files_layout(tmp_path_factory: pytest.TempPathFactory,
                                     l2_lr_ssh_files: list[str]) -> Path:
    root = tmp_path_factory.mktemp('l2_lr_ssh')
...
@pytest.fixture(scope='session')
def l2_lr_ssh_dir(tmpdir_factory: pytest.TempdirFactory,
    ...
    for file in l2_lr_ssh_files:
        ...
        if 'Basic' in relative_path.name:
            ...
            l2_lr_ssh_unsmoothed_dataset.to_netcdf(path,
                                                   group='right',
                                                   mode='a')
...
@pytest.fixture(scope='session')
def l2_lr_ssh_basic_files_unique(
        l2_lr_ssh_basic_files: list[Path]) -> list[Path]:
    return l2_lr_ssh_basic_files


@pytest.fixture(scope='session')
def l2_lr_ssh_expert_files_unique(
        l2_lr_ssh_expert_files: list[Path]) -> list[Path]:
