...
@pytest.fixture(scope='session')
def l3_lr_ww_light_files(tmp_path_factory: pytest.TempPathFactory,
                         _light_metadata: GroupMetadata) -> Path:
...
def l3_lr_ww_light_dataset(_light_metadata: GroupMetadata) -> xr.Dataset:
    ds = group_metadata_to_xarray(_light_metadata,
                                  LightExtendedSubsetGenerator())
...
@pytest.fixture(scope='session')
def l3_lr_ww_extended_files(tmp_path_factory: pytest.TempPathFactory,
                            _extended_metadata: GroupMetadata) -> Path:
    tmp_path = tmp_path_factory.mktemp('l3_karin_wind_wave')
...
def l3_lr_ww_extended_dataset(_extended_metadata: GroupMetadata) -> xr.Dataset:
    ds = group_metadata_to_xarray(
        pluck(_extended_metadata, '/tile_10km/box_40km'),
        LightExtendedSubsetGenerator())
    return ds.set_coords(['longitude', 'latitude'])
...
@pytest.fixture(scope='session')
def l3_lr_ww_dir_layout(tmp_path_factory: pytest.TempPathFactory,
                        l3_lr_ww_files: list[str]) -> Path:
