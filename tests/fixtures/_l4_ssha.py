...
def _build_grid(lon: np_t.NDArray[np.int64 | np.float64],
    ...
    ds = xr.Dataset(data_vars=dict(time=('time', time.astype('M8[ns]')),
                                   sla=(('time', lon_name, lat_name), sla)),
                    coords={
                        lon_name: ((lon_name), lon),
                        lat_name: ((lat_name), lat)
                    })
    return ds
...
@pytest.fixture(scope='session')
def l4_ssha_dir_layout(tmp_path_factory: pytest.TempPathFactory,
                       l4_ssha_files: list[str]) -> Path:
...
@pytest.fixture(scope='session')
def l4_ssha_dir(tmp_path_factory: pytest.TempPathFactory,
                l4_ssha_files_2: list[str]) -> Path:
    # create test folder
