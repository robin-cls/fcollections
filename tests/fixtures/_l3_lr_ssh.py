...
def _l3_lr_ssh_basic_metadata() -> GroupMetadata:
    return GroupMetadata(name='/',
                         variables=[
                             VariableMetadata('time', np.dtype('M8[ns]'),
                                              ('num_lines', ), {}),
                             VariableMetadata('longitude', float,
                                              ('num_lines', 'num_pixels'), {}),
                             VariableMetadata('latitude', float,
                                              ('num_lines', 'num_pixels'), {}),
                             VariableMetadata('ssha_noiseless', float,
                                              ('num_lines', 'num_pixels'), {}),
                             VariableMetadata('i_num_line', int,
                                              ('num_nadir', ), {}),
                             VariableMetadata('i_num_pixel', int,
                                              ('num_nadir', ), {}),
                         ],
                         dimensions={
                             'num_lines': 9860,
...
class BasicGenerator(HalfOrbitTrackCoordinatesGenerator):
    ...
    def ssha_noiseless(self, shape: tuple[int,
...
@pytest.fixture(scope='session')
def l3_lr_ssh_basic_dataset(
        _l3_lr_ssh_basic_metadata: GroupMetadata) -> xr.Dataset:
    generator = BasicGenerator(*_swath_properties(25),
...
def _l3_lr_ssh_expert_metadata(
        _l3_lr_ssh_basic_metadata: GroupMetadata) -> GroupMetadata:
...
class ExpertGenerator(BasicGenerator):
    ...
    def cross_track_distance(self,
                             shape: tuple[int]) -> np_t.NDArray[np.float64]:
        return np.arange(-(shape[0] - 1) * 1000, shape[0] * 1000 + 1,
                         2000).astype(float)


@pytest.fixture(scope='session')
def l3_lr_ssh_expert_dataset(
        _l3_lr_ssh_expert_metadata: GroupMetadata) -> xr.Dataset:
    generator = ExpertGenerator(*_swath_properties(25),
                                t0=np.datetime64('2024-01-01T11:39'),
                                dt=np.timedelta64(100, 'ms'))
...
@pytest.fixture(scope='session')
def l3_lr_ssh_dir_empty_files(tmp_path_factory: pytest.TempPathFactory,
                              l3_lr_ssh_files: list[str]) -> Path:
...
@pytest.fixture(scope='session')
def l3_lr_ssh_dir_empty_files_layout(tmp_path_factory: pytest.TempPathFactory,
                                     l3_lr_ssh_files: list[str]) -> Path:
    root = tmp_path_factory.mktemp('l3_lr_ssh')
...
def l3_lr_ssh_dir(
    ...
    # 'datasets' fixtures
    basic_generator = BasicGenerator(*_swath_properties(25),
                                     t0=np.datetime64('2024-01-01T11:39'),
                                     dt=np.timedelta64(100, 'ms'),
                                     half_orbit_numbers=itertools.product(
                                         [531, 532], [25, 26]))
    ...
    expert_generator = ExpertGenerator(
        *_swath_properties(25),
        t0=np.datetime64('2024-01-01T11:39'),
        dt=np.timedelta64(100, 'ms'),
        half_orbit_numbers=iter([(532, 25),
                                 *itertools.product([532, 533], [25, 26])]))
    ...
    for file, _ in zip(l3_lr_ssh_files[:4], basic_generator, strict=True):
        ...
        basic_dataset = group_metadata_to_xarray(_l3_lr_ssh_basic_metadata,
                                                 basic_generator)
    ...
    for file, _ in zip(l3_lr_ssh_files[4:9], expert_generator, strict=True):
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        expert_dataset = group_metadata_to_xarray(_l3_lr_ssh_expert_metadata,
                                                  expert_generator)
...
@pytest.fixture(scope='session')
def l3_lr_ssh_basic_files_unique(
        l3_lr_ssh_basic_files: list[Path]) -> list[list[Path]]:
    return l3_lr_ssh_basic_files


@pytest.fixture(scope='session')
def l3_lr_ssh_expert_files_unique(
        l3_lr_ssh_expert_files: list[Path]) -> list[list[Path]]:
