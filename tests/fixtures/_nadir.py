...
def _build_along_track(lon: np_t.NDArray[np.int64 | np.float64],
                       lat: np_t.NDArray[np.int64 | np.float64],
                       lon_name: str = 'longitude',
                       lat_name: str = 'latitude') -> xr.Dataset:
    np.random.seed(0)
    ds = xr.Dataset()
...
@pytest.fixture(scope='session')
def l3_nadir_dir_layout(tmp_path_factory: pytest.TempPathFactory,
                        l3_nadir_files: list[str]) -> Path:
...
@pytest.fixture(scope='session')
def l3_nadir_dir(tmp_path_factory: pytest.TempPathFactory,
                 l3_nadir_files_2: list[str],
                 l3_nadir_dataset_0_360: xr.Dataset) -> Path:
    # create test folder
...
@pytest.fixture(scope='session')
def l2_nadir_dir(tmp_path_factory: pytest.TempPathFactory,
                 l2_nadir_files: list[str]) -> Path:
    # create test folder
    test_dir = tmp_path_factory.mktemp('test_dir')
