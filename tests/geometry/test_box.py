...
@pytest.mark.parametrize('box, expected', [
    ...
    (((-97.8, 25.3), (-96.8, 28.4)), ((-98.4375, 23.90625),
                                      (-95.625, 29.53125))),
    (((5.9, 4.2), (7.5, 4.8)), ((5.625, 2.8125), (8.4375, 5.625))),
],
                         ids=[
                             'bigger_than_precision', 'long_vertical_box',
                             'smaller_than_precision'
                         ])
def test_expand_box(box: tuple[tuple[float, float], tuple[float, float]],
    ...
    expected = pyi_geod.Box(pyi_geod.Point(*expected[0]),
                            pyi_geod.Point(*expected[1]))

    actual = expand_box(box, precision=3)
    assert (actual.min_corner.lon == expected.min_corner.lon
