...
@pytest.mark.parametrize('phase, pass_numbers', [
    ('bad', [1]),
    ('calval', [29]),
    ('science', [585]),
])
...
@pytest.mark.parametrize('phase, pass_numbers', [('calval', [25, 26]),
                                                 ('science', [532, 579])])
def test_query_geometries(phase, pass_numbers):
    swath_geom = query_geometries(pass_numbers, phase)
    assert list(swath_geom.sort_values('pass_number').pass_number) == list(
        pass_numbers)
    assert len(swath_geom.geometry) == len(pass_numbers)


@pytest.mark.parametrize('phase, bbox, result_passes', [
...
@pytest.mark.parametrize('phase, bbox', [
    ('science', (0, 0, 0, 0)),
