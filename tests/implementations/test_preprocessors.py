...
def test_preprocessor_cycle_pass_error_bad_filename():
    ds = xr.Dataset()
    ds.encoding[
        'source'] = 'L2_LR_SSH_Basic_20230608T191826_20230608T200933_PIB0_01.nc'
...
def test_preprocessor_cycle_pass_error_missing_dimension():
    ds = xr.Dataset()
    ds.encoding[
        'source'] = 'SWOT_L2_LR_SSH_Basic_546_011_20230608T191826_20230608T200933_PIB0_01.nc'
...
def test_preprocessor_cycle_pass(filename: str):
    ds = xr.Dataset(data_vars=dict(ssha=(('num_lines', 'num_pixels'),
                                         np.ones((10, 3)))))
...
@pytest.mark.parametrize('variables_to_remove', [
    ['i_num_line', 'i_num_pixel'],
    ['i_num_line'],
    ['i_num_line'],
])
def test_preprocessor_extract_nadir_missing_indexes(
...
def test_preprocessor_unclip_nadir():
    ds = xr.Dataset(
        data_vars={
            'sla': (('num_lines', 'num_pixels'), np.random.random((30, 4))),
            'sla_noiseless': (('num_lines', 'num_pixels'),
