...
from fcollections.implementations import FileNameConventionDAC, NetcdfFilesDatabaseDAC
...
def expected_times() -> np_t.NDArray[np.datetime64]:
    return np.array(
        ['2023-01-02T12', '2023-01-04', '2023-01-05T12', '2023-01-05T18'],
        dtype='M8')


@pytest.mark.parametrize('time, result_slice', [
    (None, slice(0, None)),
    (('2023-01-03', '2023-01-07'), slice(1, None)),
])
def test_list_dac(dac_dir: Path, expected_times: np_t.NDArray[np.datetime64],
                  time: bool, result_slice: slice):
...
def test_generate_string():
    ...
    assert conv.generate(
        time=np.datetime64('2023-01-03')) == 'dac_dif_26665_00.nc'
