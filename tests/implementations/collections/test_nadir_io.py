...
@pytest.mark.parametrize('time, mission, result_size', [
    (None, None, 3),
...
def test_list_l3_nadir(l3_nadir_dir: Path, time: None | np.datetime64,
                       mission: None | MissionsPhases, result_size: int):
...
@pytest.mark.parametrize('args, result_size', [
    ({}, 3),
    ({
        'time': (np.datetime64('2023-07-07'), np.datetime64('2023-07-08'))
    }, 2),
    ({
        'cycle_number': 575
    }, 2),
...
def test_list_l2_nadir(l2_nadir_dir: Path, args: dict[str, tp.Any],
                       result_size: int):
...
def parsing_result() -> pda.DataFrame:
    return pda.DataFrame([
        (MissionsPhases.j3n, ProductLevel.L3, 1,
         Period('2023-01-01', '2023-01-02',
                include_stop=False), np.datetime64('2023-01-03'), 'nrt_1.nc'),
        (MissionsPhases.j3n, ProductLevel.L3, 1,
         Period('2023-01-01', '2023-01-02',
                include_stop=False), np.datetime64('2023-01-04'), 'nrt_2.nc'),
        (MissionsPhases.j3n, ProductLevel.L3, 1,
         Period('2023-01-01', '2023-01-02',
                include_stop=False), np.datetime64('2023-01-05'), 'nrt_3.nc'),
        (MissionsPhases.j3n, ProductLevel.L3, None,
         Period('2023-01-01', '2023-01-02',
                include_stop=False), np.datetime64('2023-01-05'), 'nrt_4.nc'),
        (MissionsPhases.j3n, ProductLevel.L3, 1,
         Period('2023-01-02', '2023-01-03',
                include_stop=False), np.datetime64('2023-01-06'), 'nrt_5.nc'),
        (MissionsPhases.j3n, ProductLevel.L3, 1,
         Period('2023-01-02', '2023-01-03',
                include_stop=False), np.datetime64('2023-01-07'), 'nrt_6.nc'),
        (MissionsPhases.al, ProductLevel.L3, 1,
         Period('2023-01-01', '2023-01-02',
                include_stop=False), np.datetime64('2023-01-04'), 'nrt_7.nc'),
        (MissionsPhases.al, ProductLevel.L3, 1,
         Period('2023-01-01', '2023-01-02',
                include_stop=False), np.datetime64('2023-01-05'), 'nrt_8.nc'),
    ],
                         columns=[
                             'mission', 'product_level', 'resolution', 'time',
                             'production_date', 'filename'
                         ])


def test_deduplicate_error_two_missions(l3_nadir_dir: Path,
                                        parsing_result: pda.DataFrame):
    db = NetcdfFilesDatabaseL3Nadir(l3_nadir_dir)
    with pytest.raises(ValueError):
...
def test_validate(l3_nadir_dir: Path, parsing_result: pda.DataFrame):
    db = NetcdfFilesDatabaseL3Nadir(l3_nadir_dir)
    df_sub = parsing_result[(parsing_result['mission'] == MissionsPhases.j3n)
                            & (parsing_result['resolution'] == 1)].copy()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (MissionsPhases.s6a_hr, Delay.MY,
...
def test_generate_layout(mission: MissionsPhases, delay: Delay, expected: str):
    path = CMEMS_NADIR_SSHA_LAYOUT.generate('/SEALEVEL_GLO_PHY_L3_NRT_008_044',
                                            year=1995,
                                            month=5,
                                            resolution=1,
                                            delay=delay,
...
def test_layout_l3(l3_nadir_dir_layout: Path, expected: list[int],
                   filters: dict[str, tp.Any], l3_nadir_files: list[str]):
    """Test layout filters not integrated in the database."""
    fd = FileDiscoverer(FileNameConvention(re.compile(r'.*'), []),
                        FileSystemIterable(layout=CMEMS_NADIR_SSHA_LAYOUT))

    actual = {
        os.path.basename(f)
        for f in fd.list(l3_nadir_dir_layout, **filters).filename
    }
...
def test_list_l3_layout(l3_nadir_dir_layout: Path, filters: dict[str, tp.Any]):
    db = NetcdfFilesDatabaseL3Nadir(l3_nadir_dir_layout,
                                    layout=CMEMS_NADIR_SSHA_LAYOUT)
