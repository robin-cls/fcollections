...
class TestConvention:
    ...
    def test_internal_convention_generate(self):
        ...
        period = Period(np.datetime64('2024-01-02'),
    ...
    def test_internal_convention_parse(self):
        ...
        reference = (Period(np.datetime64('2024-01-02'),
                            np.datetime64('2024-01-03'),
                            include_stop=False), )
...
class TestListing:
    ...
    @pytest.mark.parametrize(
        'params, result_size',
        [({}, 3), ({
            'time': np.datetime64('2023-10-16')
        }, 2), ({
            'production_date': np.datetime64('2023-11-06')
        }, 1), ({
            'delay': Delay.NRT
        }, 2)])
    def test_list_gridded_sla(self, l4_ssha_dir: Path,
                              params: dict[str, tp.Any], result_size: int):
...
class TestLayout:
    ...
    def test_generate_layout(self):
        ...
        assert path == '/duacs-experimental/dt-phy-grids/l4_karin_nadir/v1.0/miost/science'

    @pytest.mark.parametrize('filters, expected', [({}, [0, 1, 2, 3]),
                                                   ({
                                                       'version': '0.3'
                                                   }, [2, 3]),
                                                   ({
    ...
    def test_list_layout(self, l4_ssha_dir_layout: Path,
                         l4_ssha_files: list[str], expected: list[int],
                         filters: dict[str, tp.Any]):

        fd = FileDiscoverer(FileNameConventionGriddedSLA(),
                            FileSystemIterable(layout=AVISO_L4_SWOT_LAYOUT))

        actual = {
            os.path.basename(f)
            for f in fd.list(l4_ssha_dir_layout, **filters).filename
        }
