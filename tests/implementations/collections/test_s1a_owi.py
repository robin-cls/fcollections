...
@pytest.mark.parametrize('params, result_size', [
    ({}, 2),
    ({
        'acquisition_mode': AcquisitionMode.EW
    }, 0),
    ({
        'acquisition_mode': AcquisitionMode.IW
    }, 2),
    ({
        'slice_post_processing': S1AOWISlicePostProcessing.OCN
    }, 1),
    ({
        'time': (np.datetime64('2024-09-24T11:21'), None)
    }, 1),
    ({
        'resolution': 3
    }, 2),
    ...
    ({
        'product_type': S1AOWIProductType.SW
    }, 1),
])
def test_list_s1aowi(s1aowi_dir: Path, params: dict[str, tp.Any],
                     result_size: int):
