...
def brute_force_geographical_selection(ds: xr.Dataset, lon_min: float,
                                       lat_min: float, lon_max: float,
                                       lat_max: float) -> xr.Dataset:
    mask = ((ds['longitude'] >= lon_min) & (ds['longitude'] <= lon_max) &
    ...
    to_crop = {
        v
        for v in ds.variables if set(mask.dims) & set(ds[v].dims) != set()
    }
...
def extract_box_from_polygon(
    ...
    bbox = lon_center - box_size, lat_center - box_size, lon_center + box_size, lat_center + box_size
    return bbox
