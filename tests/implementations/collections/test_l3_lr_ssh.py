...
class TestReader:
    ...
    @pytest.mark.parametrize(
        'subset',
        [ProductSubset.WindWave, ProductSubset.Light, ProductSubset.Extended])
    def test_invalid_subset(self, subset: ProductSubset,
                            l3_lr_ssh_basic_files: list[str]):
        with pytest.raises(ValueError):
            self.reader.read(subset, l3_lr_ssh_basic_files)

    def test_invalid_stack(self, l3_lr_ssh_basic_files: list[str]):
        with pytest.raises(ValueError):
            self.reader.read(ProductSubset.Basic,
                             l3_lr_ssh_basic_files,
                             stack=True)

    @pytest.mark.parametrize(
        'files, reference, half_orbit, subset',
        [('l3_lr_ssh_basic_files', 'l3_lr_ssh_basic_dataset',
          (531, 25), ProductSubset.Basic),
         ('l3_lr_ssh_expert_files', 'l3_lr_ssh_expert_dataset',
          (532, 25), ProductSubset.Expert),
         ('l3_lr_ssh_unsmoothed_files', 'l3_lr_ssh_unsmoothed_dataset',
          (10, 532), ProductSubset.Unsmoothed)])
    def test_all_read(self, files: list[str], reference: str,
                      half_orbit: tuple[int, int], subset: ProductSubset,
        ...
        ds = self.reader.read(subset,
                              files,
                              nadir=True,
                              swath=True,
                              stack=StackLevel.NOSTACK)

        xr.testing.assert_equal(
            reference,
            ds.drop_vars(['cycle_number', 'pass_number']).compute())
    ...
    def test_all_multi_read(self, files: list[str], subset: ProductSubset,
                            request: pytest.FixtureRequest):
        """Test nominal reading of Basic and expert files."""
    ...
    def test_expert_bbox(self, files: str, stack: bool,
                         request: pytest.FixtureRequest):
        pass_number = 25
        bbox = extract_box_from_polygon(pass_number)
        files = request.getfixturevalue(files)
        reference = self.reader.read(subset=ProductSubset.Basic,
                                     files=files[:1],
                                     stack=stack,
                                     nadir=False).compute()
        ...
        reference_cropped = brute_force_geographical_selection(
            reference, *bbox)
        ...
        actual_cropped = self.reader.read(subset=ProductSubset.Basic,
                                          files=files[:1],
                                          stack=stack,
                                          bbox=bbox).compute()
    ...
    @pytest.mark.parametrize('files, subset, expected_half_orbits', [
        ('l3_lr_ssh_basic_files_unique', ProductSubset.Basic, [(531, 25),
                                                               (531, 26),
                                                               (532, 25),
                                                               (532, 26)]),
        ('l3_lr_ssh_expert_files_unique', ProductSubset.Expert, [(532, 25),
                                                                 (532, 26),
                                                                 (533, 25),
                                                                 (533, 26)]),
    ])
    def test_expert_multi_read_stack_cycles(
            self, files: list[list[str]], subset: ProductSubset,
        ...
        for cycle_number in expected_cycles:
            ...
            reference_cycle['cycle_number'] = np.unique(
                reference_cycle.cycle_number.values).item()

            mask = xr.apply_ufunc(lambda p: p in valid_pass_numbers,
                                  ds_cycle['pass_number'],
                                  vectorize=True)
            ds_valids = ds_cycle.where(mask, drop=True)
            xr.testing.assert_equal(reference_cycle, ds_valids)

            mask = xr.apply_ufunc(lambda p: p in missing_pass_numbers,
                                  ds_cycle['pass_number'],
                                  vectorize=True)
    ...
    def test_expert_multi_read_stack_cycles_passes(
        ...
        for file_half_orbit, (cycle_number,
                              pass_number) in zip(files,
                                                  expected_half_orbits,
                                                  strict=True):
            ds_half_orbit = self.reader.read(subset, [file_half_orbit])
            ds_half_orbit = ds_half_orbit.set_coords(
                ['cycle_number', 'pass_number'])

            ds_half_orbit['cycle_number'] = np.unique(
                ds_half_orbit.cycle_number.values).item()
            ds_half_orbit['pass_number'] = np.unique(
                ds_half_orbit.pass_number.values).item()

            reference_cycle = ds.sel(cycle_number=cycle_number,
        ...
        missing_half_orbits = set(
            itertools.product(expected_cycles,
                              expected_passes)) - set(expected_half_orbits)
        for cycle_number, pass_number in missing_half_orbits:
            assert np.all(
                np.isnan(
                    ds.sel(cycle_number=cycle_number,
                           pass_number=pass_number)))
    ...
    @pytest.mark.parametrize('stack',
                             [StackLevel.CYCLES, StackLevel.CYCLES_PASSES])
    def test_expert_multi_read_nadir_error(self, files: list[list[str]],
        ...
        with pytest.raises(ValueError):
            ...
            self.reader.read(subset,
                             files,
                             stack=stack,
                             nadir=True,
                             swath=False)

    @pytest.mark.parametrize('swath, nadir', [(True, True), (True, False),
                                              (False, True)])
    def test_expert_selection_variables(
            self, l3_lr_ssh_expert_files: list[list[str]], swath: bool,
            nadir: bool):
        """Tests we can select the read variables."""
        selected = ['longitude', 'ssha_noiseless']
        dropped = ['cross_track_distance', 'latitude']

        ds = self.reader.read(ProductSubset.Expert,
    ...
    def test_expert_nadir_only(self, l3_lr_ssh_basic_files: list[str]):
        ds = self.reader.read(ProductSubset.Basic,
                              l3_lr_ssh_basic_files,
                              nadir=True,
                              swath=False)
    ...
    def test_unsmoothed_selected_variables(
            self, l3_lr_ssh_unsmoothed_files: list[str]):
        selected = ['longitude', 'ssha_noiseless']
        dropped = ['cross_track_distance', 'latitude']

        ds = self.reader.read(ProductSubset.Unsmoothed,
    ...
    @pytest.mark.parametrize('subset', [
        ProductSubset.Basic, ProductSubset.Expert, ProductSubset.Unsmoothed,
...
class TestListing:
    ...
    @pytest.mark.parametrize('query, half_orbits', [
        ({}, [(531, 25), (531, 26), (532, 25), (532, 26), (532, 25), (532, 25),
              (532, 26), (533, 25), (533, 26), (10, 532)]),
        ({
            'cycle_number': [532]
        }, [(532, 25), (532, 26), (532, 25), (532, 25),
            (532, 26)]), ({
                'pass_number': [532]
            }, [(10, 532)]),
        ({
            'time':
            (np.datetime64('2024-01-25T03'), np.datetime64('2024-01-25T03:30'))
        }, [(10, 532)]),
        ({
    ...
    def test_list(self, l3_lr_ssh_dir_empty_files: Path,
                  query: dict[str, tp.Any], half_orbits: list[tuple[int,
                                                                    int]]):
        ...
        actual_half_orbits = sorted([
            tuple(x)
            for x in files[['cycle_number', 'pass_number']].to_numpy()
        ])
        assert actual_half_orbits == sorted(half_orbits)

    @pytest.mark.with_geo_packages
    @pytest.mark.parametrize('bbox, pass_numbers', [
        ((-180, -90, 180, 90), [25, 26, 532]),
        ((0, -90, 360, 90), [25, 26, 532]),
    ])
    def test_list_bbox_global(self, l3_lr_ssh_dir_empty_files: Path,
                              bbox: tuple[int, int, int,
                                          int], pass_numbers: list[int]):
    ...
    def test_list_bbox(self, l3_lr_ssh_dir_empty_files: Path, pass_number: int,
                       latitude: float, pass_numbers: list[int]):
    ...
    def test_list_unmix_autopick_version(self,
        ...
        db = NetcdfFilesDatabaseSwotLRL3(l3_lr_ssh_dir_empty_files)
        files_metadata = db.list_files(subset='Expert',
                                       cycle_number=532,
                                       pass_number=25)
        assert {str(x)
                for x in files_metadata['version']} == {'1.0.2', '2.0.1'}
...
class TestQuery:
    ...
    @pytest.mark.parametrize('stack', StackLevel)
    def test_query_empty_result(self, l3_lr_ssh_basic_dir: Path,
                                stack: StackLevel):
...
class TestLayout:
    ...
    @pytest.mark.parametrize('temporality, expected', [
        (Temporality.REPROC,
    ...
    def test_list_swot_lr_l3_layout(self,
        ...
        db = NetcdfFilesDatabaseSwotLRL3(l3_lr_ssh_dir_empty_files_layout,
                                         layout=AVISO_L3_LR_SSH_LAYOUT)
        db_no_layout = NetcdfFilesDatabaseSwotLRL3(
