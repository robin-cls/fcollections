...
def periods() -> List[Period]:
    ...
    holes = np.array([
        '2023-04-24', '2023-05-01', '2023-05-02', '2023-05-03', '2023-05-20',
...
def reduced_periods() -> List[Period]:
    return [
        Period(np.datetime64('2023-01-16'),
               np.datetime64('2023-04-24'),
               include_stop=False),
        Period(np.datetime64('2023-04-25'),
               np.datetime64('2023-05-01'),
               include_stop=False),
        Period(np.datetime64('2023-05-04'),
               np.datetime64('2023-05-20'),
               include_stop=False),
        Period(np.datetime64('2023-05-22'),
               np.datetime64('2023-06-10'),
               include_stop=False),
        Period(np.datetime64('2023-06-11'),
...
def holes() -> List[Period]:
    return [
        Period(np.datetime64('2023-04-24'),
               np.datetime64('2023-04-25'),
               include_stop=False),
        Period(np.datetime64('2023-05-01'),
               np.datetime64('2023-05-04'),
               include_stop=False),
        Period(np.datetime64('2023-05-20'),
               np.datetime64('2023-05-22'),
               include_stop=False),
        Period(np.datetime64('2023-06-10'),
               np.datetime64('2023-06-11'),
               include_stop=False)
    ]


def test_fuse_successive_periods(periods: List[Period],
                                 reduced_periods: List[Period]):
    assert reduced_periods == fuse_successive_periods(periods)


def test_envelop(periods: List[Period]):
    expected = Period(np.datetime64('2023-01-16'),
                      np.datetime64('2023-07-10'),
                      include_stop=False)
