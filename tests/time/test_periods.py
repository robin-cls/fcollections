...
@pytest.mark.parametrize('tested, expected', [
    ...
    (Period(np.datetime64('2023-01-01'), np.datetime64('2023-02-25')),
     (False, False, True, False, False, False, True, True, True, False)),
                         ...
                         ids=[
                             'same', 'enclosed', 'enclosing', 'before',
                             'before_overlap', 'after', 'after_overlap'
                         ])
...
@pytest.mark.parametrize('tested, expected', [
    (Period(np.datetime64('2023-01-01'), np.datetime64('2023-02-01'), True,
            True), True),
    (Period(np.datetime64('2023-01-01'), np.datetime64('2023-02-01'), False,
            True), True),
    (Period(np.datetime64('2023-01-01'), np.datetime64('2023-02-01'), True,
            False), True),
    (Period(np.datetime64('2023-01-01'), np.datetime64('2023-02-01'), False,
...
def test_intersects_same_periods(reference: Period, tested: Period,
                                 expected: bool):
...
@pytest.mark.parametrize('tested, expected', [
    (Period(np.datetime64('2022-01-01'), np.datetime64('2023-01-01'), True,
            True), True),
    (Period(np.datetime64('2022-01-01'), np.datetime64('2023-01-01'), False,
            True), True),
    (Period(np.datetime64('2022-01-01'), np.datetime64('2023-01-01'), True,
            False), False),
    (Period(np.datetime64('2022-01-01'), np.datetime64('2023-01-01'), False,
            False), False),
    (Period(np.datetime64('2023-02-01'), np.datetime64('2023-03-01'), True,
            True), True),
    (Period(np.datetime64('2023-02-01'), np.datetime64('2023-03-01'), False,
            True), False),
    (Period(np.datetime64('2023-02-01'), np.datetime64('2023-03-01'), True,
            False), True),
    (Period(np.datetime64('2023-02-01'), np.datetime64('2023-03-01'), False,
...
def test_intersects_common_bound_ext(reference: Period, tested: Period,
                                     expected: tuple):
    assert reference.intersects(tested) == expected
    assert tested.intersects(reference) == expected
...
@pytest.mark.parametrize(
    'tested, expected',
    [(Period(np.datetime64('2023-01-01'), np.datetime64('2023-01-15'), True,
             True), True),
     (Period(np.datetime64('2023-01-01'), np.datetime64('2023-01-15'), False,
             False), True),
     (Period(np.datetime64('2023-01-01'), np.datetime64('2023-03-01'), True,
             True), True),
     (Period(np.datetime64('2023-01-01'), np.datetime64('2023-03-01'), False,
             False), True),
     (Period(np.datetime64('2023-01-15'), np.datetime64('2023-02-01'), True,
             True), True),
     (Period(np.datetime64('2023-01-15'), np.datetime64('2023-02-01'), False,
             False), True),
     (Period(np.datetime64('2022-01-01'), np.datetime64('2023-02-01'), True,
             True), True),
     (Period(np.datetime64('2022-01-01'), np.datetime64('2023-02-01'), False,
             False), True)])
def test_intersects_common_bound_int(reference: Period, tested: Period,
                                     expected: tuple):
    assert reference.intersects(tested) == expected
    assert tested.intersects(reference) == expected
...
@pytest.mark.parametrize('tested, expected', [
    (Period(np.datetime64('2023-01-15'), np.datetime64('2023-01-16'), True,
            True), True),
    (Period(np.datetime64('2023-01-15'), np.datetime64('2023-01-16'), False,
            False), True),
    (Period(np.datetime64('2022-01-02'), np.datetime64('2023-03-01'), True,
            True), True),
    (Period(np.datetime64('2022-01-02'), np.datetime64('2023-03-01'), False,
            False), True),
])
def test_intersects_int_ext(reference: Period, tested: Period,
                            expected: tuple):
    assert reference.intersects(tested) == expected
    assert tested.intersects(reference) == expected
...
def test_sort(periods: list[Period]):
    ...
    assert sorted(periods,
                  reverse=True) == [periods[ii] for ii in [2, 0, 1, 3]]
...
def test_repr():
    period = Period('2023-01-01',
                    '2023-02-01T01:02:03.004',
                    include_stop=False)
...
def test_center():
    assert Period(
        ...
        np.datetime64('2023-01-02')).center == np.datetime64('2023-01-01T12')


@pytest.mark.parametrize('period1, period2, expected', [
...
def test_union(period1: tuple[str, str, bool,
                              bool], period2: tuple[str, str, bool, bool],
               expected: tuple[str, str, bool, bool]):
    period1 = Period(np.datetime64(period1[0]), np.datetime64(period1[1]),
                     period1[2], period1[3])
    period2 = Period(np.datetime64(period2[0]), np.datetime64(period2[1]),
                     period2[2], period2[3])
    expected = Period(np.datetime64(expected[0]), np.datetime64(expected[1]),
...
@pytest.mark.parametrize('period1, period2, expected',
                         [(('2023-01-01', '2023-02-01', True, False),
                          ...
                          (('2023-01-01', '2023-02-01', True, True),
                           ...
                           ('2023-01-01', '2023-02-01', True, True)),
                          (('2023-01-01', '2023-02-01', False, True),
                           ('2024-01-01', '2024-02-01', False, True), None),
                          (('2023-01-01', '2023-02-01', True, True),
                           ('2023-02-01', '2024-02-01', True, True),
                           ('2023-02-01', '2023-02-01', True, True)),
                          (('2023-01-01', '2023-02-01', True, False),
                           ('2023-02-01', '2024-02-01', True, True), None),
                          (('2023-01-01', '2023-02-01', True, True),
                           ('2023-02-01', '2024-02-01', False, True), None)])
def test_intersection(period1: tuple[str, str, bool, bool],
    ...
    period2 = Period(np.datetime64(period2[0]), np.datetime64(period2[1]),
                     period2[2], period2[3])
    expected = Period(np.datetime64(expected[0]), np.datetime64(
...
def test_intersection_time(reference: Period, time: np_t.NDArray[np.float64],
