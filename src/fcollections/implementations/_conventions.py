...
DESCRIPTIONS = {
    ...
    'Delay.',
    'oc_variable':
    'Ocean color variable.',
    'temporal_resolution':
    'Temporal resolution, such as P1D, P1M.',
    'spatial_resolution':
    'Spatial resolution, such as 4km, 1km, 300M.',
    'version':
...
class FileNameConventionSwotL3WW(FileNameConvention):
    ...
    def __init__(self):
        super().__init__(
            ...
            generation_string=
            'SWOT_L3_LR_WIND_WAVE_{subset!f}_{cycle_number:>03d}_{pass_number:>03d}_{time!f}_v{version}.nc'
        )
    ...
    def __init__(self):
        super().__init__(
            ...
            fields=[
                ...
                FileNameFieldString(
                    'spatial_resolution',
                    description=DESCRIPTIONS['spatial_resolution']),
                FileNameFieldString(
                    'temporal_resolution',
...
AVISO_L2_LR_SSH_LAYOUT = Layout([
    FileNameConvention(re.compile(r'(?P<version>P[I|G][A-Z]\d{1})'),
                       [_ADAPTED_L2_FIELD], '{version!f}'),
    FileNameConvention(re.compile(r'(?P<subset>.*)'),
                       [FileNameConventionSwotL2().get_field('subset')],
                       '{subset}'),
    FileNameConvention(re.compile(r'cycle_(?P<cycle_number>\d{3})'),
...
_AVISO_L3_LR_SSH_LAYOUT_V2 = Layout([
    FileNameConvention(re.compile(r'v(?P<version>.*)'),
...
_AVISO_L3_LR_SSH_LAYOUT_V3 = Layout([
    *_AVISO_L3_LR_SSH_LAYOUT_V2.conventions[:2],
    FileNameConvention(re.compile(r'(?P<temporality>reproc|forward)'), [
        FileNameFieldEnum('temporality',
                          Temporality,
                          case_type_encoded=CaseType.lower,
                          case_type_decoded=CaseType.upper),
    ], '{temporality!f}'),
...
AVISO_L4_SWOT_LAYOUT = Layout([
    FileNameConvention(re.compile(r'v(?P<version>.*)'),
                       [FileNameFieldString('version')], 'v{version!f}'),
    FileNameConvention(re.compile(r'(?P<method>4dvarnet|4dvarqg|miost)'),
                       [FileNameFieldString('method')], '{method}'),
    FileNameConvention(re.compile(r'(?P<phase>.*)'),
...
CMEMS_NADIR_SSHA_LAYOUT = Layout([
    ...
    FileNameConvention(re.compile(r'(?P<year>\d{4})'),
                       [FileNameFieldInteger('year')], '{year}'),
    FileNameConvention(re.compile(r'(?P<month>\d{2})'),
                       [FileNameFieldInteger('month')], '{month:0>2d}')
])
