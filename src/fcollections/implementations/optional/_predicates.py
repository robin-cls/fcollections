...
class SwotGeometryPredicate(IPredicate):
    ...
    def __init__(self, indexes: tuple[int, int], bbox: tuple[float, float,
                                                             float, float]):

        self.cycle_number_index, self.pass_number_index = indexes

        def selected(cycle_number: int, pass_number: int,
                     cycle_range: tuple[int, int | None],
                     selected_pass_numbers: list[int]) -> bool:
            return ((cycle_range[0] <= cycle_number) and
                    (cycle_range[1] is None or cycle_number <= cycle_range[1])
        ...
        for phase in PHASES[Missions.Swot]:
            ...
            logger.info('The bbox intersects with pass numbers (%s phase): %s',
                        phase.short_name.lower(), pass_numbers_intersect)
    ...
    def __call__(self, record: tuple[tp.Any, ...]) -> bool:
        cycle_number, pass_number = record[self.cycle_number_index], record[
            self.pass_number_index]
        return functools.reduce(lambda x, y: x or y, [
            predicate(cycle_number, pass_number)
