...
class AreaSelector1D(IAreaSelector, abc.ABC):
    ...
    def apply(self, ds: xr.Dataset, bbox: tuple[float, float, float,
                                                float]) -> xr.Dataset:
        ...
        idx = _select_2d_indices_intersect_bounds(lon, lat, bbox_lon_norm,
                                                  (lat_min, lat_max),
                                                  data_convention.lon_max)

        ds_sel = ds.isel(**self.dims_selection(idx))

        logger.info('Size of the dataset matching the bbox: %s',
                    dict(ds_sel.sizes))
...
class AreaSelector2D(IAreaSelector):
    ...
    def apply(self, ds: xr.Dataset, bbox: tuple[float, float, float,
        ...
        bbox_convention = guess_longitude_convention(
            np.array((lon_min, lon_max))).value
        ...
        if lat_slices == []:
            logger.debug('No intersection between the bbox and the dataset.')
            return ds.isel(**{
                self.longitude: slice(0),
                self.latitude: slice(0)
            })
        ...
        lon_slice = _select_slices_intersect_bounds(
            lon[slice(indx_transition, lon.size)], (lon_min, lon_max),
            indx_transition)
        if lon_slice is not None:
            lon_slices.extend([sl for sl in lon_slice if sl is not None])
        ...
        ds_sel = xr.concat([
            ds.isel(**{
                self.longitude: slice_i,
                self.latitude: lat_slice
...
def _find_indices_in_reversed(array: np.array, x: float,
...
def _select_slices_intersect_bounds(data: np.array,
                                    bounds: tuple[float, float],
                                    ind_start: int = 0) -> dict[slice]:
...
def _create_slice(data: np.array, bounds: tuple[float, float], i: int, j: int,
                  ind_start: int) -> slice:
    ...
    return slice(i + ind_start,
                 j + ind_start + 1 if j <= data.size - 1 else j + ind_start)


def _select_2d_indices_intersect_bounds(x: np.array,
                                        y: np.array,
