...
def summary():
    ...
    for name, data in _defaults().items():
        ...
        table.add_row(
            name, f'{sum(files)}/{len(files)}', ','.join(sorted(data.keys)),
            '\n'.join(map(lambda x: x.as_posix(), data.lookup_folders())))
    console.print(table)


@app.command()
def details(data_type: Annotated[
    str,
    typer.Argument(
    ...
    for key in sorted(data.keys):
        ...
        table.add_row(key, file_path.name, file_path.parent.as_posix(),
                      str(file_path.exists()))
    console.print(table)


@app.command()
def download(target_folder: Annotated[Path,
                                      typer.Argument(
                                          help='target folder for download')]):
    """Download all auxiliary data from remote sources."""
    target_folder.mkdir(parents=True, exist_ok=True)

    datas = _defaults(target_folder).values()
    with Progress() as progress:
        data_task = progress.add_task('[cyan]Processing sources...',
                                      total=len(datas))

        for data in datas:
            # Inner task: processing items in each dataset
            key_task = progress.add_task(
                f'[green]Processing keys in {data.name}...',
                total=len(data.keys))
            for key in data.keys:
                data[
                    key]  # Will trigger a download if not found on local folders
