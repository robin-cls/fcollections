...
def expand_box(box: pyi_geod.Box, precision: int) -> pyi_geod.Box:
    ...
    """
    geohashes = pyi_geoh.encode([box.min_corner.lon, box.max_corner.lon],
    ...
    for g in geohashes:
        ...
        min_lon = min(box.min_corner.lon,
                      min_lon) if min_lon is not None else box.min_corner.lon
        max_lon = max(box.max_corner.lon,
                      max_lon) if max_lon is not None else box.max_corner.lon
        min_lat = min(box.min_corner.lat,
                      min_lat) if min_lat is not None else box.min_corner.lat
        max_lat = max(box.max_corner.lat,
                      max_lat) if max_lat is not None else box.max_corner.lat
    return pyi_geod.Box(pyi_geod.Point(min_lon, min_lat),
                        pyi_geod.Point(max_lon, max_lat))


def normalize_polygon(
        polygon: Polygon,
        convention: LongitudeConvention) -> Polygon | MultiPolygon:
